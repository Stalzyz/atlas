...
files.forEach(file => {
  ...
  // Pattern 1: process.env.NEXT_PUBLIC_API_URL || 'https://api.grekam.in...'
  // Pattern 2: process.env.NEXT_PUBLIC_API_URL || "https://api.grekam.in..."
  // Pattern 3: process.env.API_URL || "https://api.grekam.in..."
  ...
  content = content.replace(regex, (match, prefix, quote1, suffix, quote2) => {
    ...
    return `${prefix}(process.env.NODE_ENV === 'development' ? ${quote1}http://localhost:6005${suffix}${quote2} : ${quote1}https://api.grekam.in${suffix}${quote2})`;
  });
