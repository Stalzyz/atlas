#!/bin/bash
# ═══════════════════════════════════════════════════════════════════
#  🚀 ATLAS FULL ENVIRONMENT RECOVERY — VPS SCRIPT
#  Run this as root on your VPS: bash /tmp/atlas_recover.sh
# ═══════════════════════════════════════════════════════════════════
set -e
...
PROJ_DIR=""
for dir in \
    "/var/www/atlas_new/current" \
    "/var/www/atlas/current" \
    "/var/www/current" \
    "/root/atlas" \
    "/root/Atlas_website" \
    "/var/www/html" \
    "/var/www/atlas_new" \
    "/var/www/atlas_app"; do
...
if [ -f "$PROJ_DIR/ecosystem.config.js" ]; then
    ...
    if [ -f "$PROJ_DIR/apps/api/dist/src/main.js" ]; then
        pm2 start "$PROJ_DIR/apps/api/dist/src/main.js" --name "atlas-api" -i 1 -- --max-old-space-size=2048
    fi
    
    # Start Storefront
    if [ -d "$PROJ_DIR/apps/storefront/.next" ]; then
        cd "$PROJ_DIR/apps/storefront"
        pm2 start npm --name "atlas-storefront" -- run start
    fi
    
    # Start Admin
    if [ -d "$PROJ_DIR/apps/admin/.next" ]; then
        cd "$PROJ_DIR/apps/admin"
        pm2 start npm --name "atlas-admin" -- run start
    fi
...
inf "Checking ports..."
for port in 4401 6005 6009 3000; do
...
for url in "http://atlas.grekam.in" "https://atlas.grekam.in" "https://atlasapi.grekam.in/api/v1/health"; do
