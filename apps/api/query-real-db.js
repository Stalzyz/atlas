...
async function main() {
  const client = new Client({ connectionString: 'postgresql://postgres@localhost:5432/atlas' });
  await client.connect();
  const res = await client.query('SELECT count(*) FROM "Product";');
  console.log("Products in atlas:", res.rows[0]);
  const res2 = await client.query('SELECT count(*) FROM "Order";');
  console.log("Orders in atlas:", res2.rows[0]);
