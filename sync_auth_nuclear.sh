...
ssh -o StrictHostKeyChecking=no root@$VPS_IP << REMOTE
  ...
  DATABASE_URL="$DB_URL" PORT=6005 pm2 start apps/api/dist/src/main.js --name atlas-api --update-env
  DATABASE_URL="$DB_URL" PORT=6010 pm2 start apps/admin/.next/standalone/apps/admin/server.js --name atlas-admin --update-env
  DATABASE_URL="$DB_URL" PORT=6009 pm2 start apps/storefront/.next/standalone/apps/storefront/server.js --name atlas-storefront --update-env
