...
ssh $SSH_OPTS root@$VPS_IP << 'REMOTE_SCRIPT'
  ...
  NODE_ENV=production PORT=6005 \
    pm2 start apps/api/dist/src/main.js --name atlas-api

  NODE_ENV=production PORT=6010 \
    pm2 start server.js \
    --name atlas-admin \
    --cwd /var/www/atlas_new/apps/admin/.next/standalone/apps/admin

  NODE_ENV=production PORT=6009 \
    pm2 start server.js \
    --name atlas-storefront \
    --cwd /var/www/atlas_new/apps/storefront/.next/standalone/apps/storefront
