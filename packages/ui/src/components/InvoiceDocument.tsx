      ...
      <div className="grid grid-cols-2 gap-12 mb-10">
        <div>
          ...
          <p className="text-gray-600 text-sm leading-relaxed">
            {customer.address?.street || customer.address?.address}<br />
            {customer.address?.city}, {customer.address?.state} {customer.address?.zip || customer.address?.zipCode}<br />
            {customer.email}<br />
            {customer.phone}
          </p>
        </div>
        <div>
          ...
          <p className="text-gray-600 text-sm leading-relaxed">
            {customer.address?.street || customer.address?.address}<br />
            {customer.address?.city}, {customer.address?.state} {customer.address?.zip || customer.address?.zipCode}
          </p>
        </div>
      </div>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b-2 border-gray-900">
              ...
              <th className="py-4 text-xs font-bold uppercase tracking-wider text-gray-900 text-right">Tax (12%)</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {items.map((item, idx) => (
              <tr key={idx}>
                ...
                <td className="py-5 text-right text-gray-900">₹{Number(item.price).toLocaleString()}</td>
                <td className="py-5 text-right text-gray-500">₹{Number(item.tax).toLocaleString()}</td>
                <td className="py-5 text-right font-medium text-gray-900">₹{Number(item.total).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="flex justify-end">
        <div className="w-64 space-y-3">
          <div className="flex justify-between text-sm">
            ...
            <span className="text-gray-900 font-medium">₹{Number(subtotal).toLocaleString()}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-gray-400">Tax Total</span>
            <span className="text-gray-900 font-medium">₹{Number(taxTotal).toLocaleString()}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-gray-400">Shipping</span>
            <span className="text-gray-900 font-medium">₹{Number(shipping).toLocaleString()}</span>
          </div>
          <div className="flex justify-between pt-3 border-t border-gray-900">
            ...
            <span className="text-lg font-bold text-[#6D0F1B]">₹{Number(total).toLocaleString()}</span>
          </div>
        </div>
      </div>
