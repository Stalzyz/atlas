...
const serverFile = '/var/www/atlas_new/apps/admin/.next/standalone/apps/admin/.next/server/app/logistics/fulfillment/page.js';
const chunkDir = '/var/www/atlas_new/apps/admin/.next/standalone/apps/admin/.next/static/chunks/app/logistics/fulfillment/';
...
if (fs.existsSync(chunkDir)) {
    const chunks = fs.readdirSync(chunkDir).filter(f => f.startsWith('page-') && f.endsWith('.js'));
    for (const chunk of chunks) {
        patchFile(path.join(chunkDir, chunk));
    }
} else {
    console.log("Chunk dir not found: " + chunkDir);
}
