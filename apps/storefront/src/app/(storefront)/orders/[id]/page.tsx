...
function OrderTrackingDetailPageInner() {
    ...
    return (
      <div className="min-h-screen bg-ivory flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
           ...
           <p className="text-[10px] uppercase font-bold tracking-[0.3em] text-primary/40">Opening your luxury files...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !order) {
    return (
      <div className="min-h-screen bg-ivory flex flex-col items-center justify-center space-y-8 px-6 text-center overflow-hidden relative">
         ...
         <div className="w-24 h-24 bg-primary/5 rounded-full flex items-center justify-center text-primary border border-primary/10 relative z-10">
         ...
         <Link href="/orders/track" className="bg-charcoal text-white px-12 py-4 rounded-full text-[10px] font-bold uppercase tracking-[0.3em] relative z-10 hover:bg-primary transition-colors shadow-2xl">
           Verify Credentials
         </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-ivory pb-24 selection:bg-primary/10">
      ...
      <div className="bg-white/50 backdrop-blur-md border-b border-charcoal/5 px-6 md:px-12 py-8 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex justify-between items-center text-charcoal">
           <Link href={email ? "/orders/track" : "/account/orders"} className="flex items-center gap-3 text-[10px] uppercase font-bold tracking-[0.3em] opacity-40 hover:opacity-100 hover:text-primary transition-all">
           ...
           <div className="w-32 hidden md:block">
             <button onClick={() => setIsInvoiceModalOpen(true)} className="flex items-center gap-2 text-[10px] uppercase font-bold tracking-[0.2em] text-primary border border-primary/20 px-4 py-2 rounded-full hover:bg-primary hover:text-white transition-all shadow-sm">
        ...
        <div className="space-y-24">
          ...
          <div className="space-y-16">
            <div className="space-y-2">
               <span className="text-[10px] bg-primary text-white px-3 py-1 rounded-full font-bold uppercase tracking-[0.3em]">{order.status}</span>
            ...
            <div className="relative pl-12 space-y-20">
               ...
               <motion.div 
                 ...
                 className="absolute left-4 top-4 w-[1px] bg-primary z-10 transition-all duration-[2000ms] ease-in-out origin-top"
               />
                 ...
                 return (
                   ...
                   >
                      <div className={`absolute -left-[32px] w-4 h-4 rounded-full border border-ivory transition-all duration-1000 ${
                        isCompleted ? "bg-primary scale-125 shadow-[0_0_15px_rgba(114,30,47,0.4)]" : "bg-white border-charcoal/10"
                      } ${isCurrent ? "ring-8 ring-primary/5" : ""}`} />
                      
                      <div className="space-y-3 flex-1 pb-4">
                         <div className="flex items-center gap-4">
                           <span className={`${isCompleted ? "text-primary" : "text-charcoal/10"}`}>{step.icon}</span>
                         ...
                         {isCurrent && isCompleted && (
                           <div className="text-[10px] text-primary font-bold uppercase tracking-widest flex items-center gap-2 mt-2">
          ...
          <div className="grid md:grid-cols-2 gap-12 pt-12 border-t border-charcoal/5">
                 ...
                 <div className="bg-white p-8 rounded-[32px] border border-charcoal/5 space-y-6 shadow-sm">
                    {order.trackingId ? (
                      <div className="space-y-4">
                        ...
                        <button className="w-full bg-charcoal text-white py-4 rounded-2xl text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-primary transition-all">Track Official</button>
                      </div>
                    ) : (
              ...
              <div className="space-y-4">
                 ...
                 <div className="bg-primary/[0.02] p-8 rounded-[32px] border border-primary/5 space-y-6 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="relative z-10 space-y-4">
                      ...
                      <button 
                         ...
                         className="flex items-center gap-3 text-[10px] uppercase font-bold tracking-widest text-primary border-b-2 border-primary/20 pb-2 hover:border-primary transition-all disabled:opacity-50"
                      >
          ...
          <div className="bg-white p-10 md:p-12 rounded-[40px] border border-charcoal/5 shadow-2xl space-y-12">
                ...
                {order.items.map((item: any) => (
                  <div key={item.id} className="flex gap-6 group">
                     ...
                     <div className="flex-1 py-1 space-y-2 flex flex-col justify-center">
                        <div>
                          <p className="text-[10px] uppercase font-bold tracking-[0.3em] text-primary mb-1">{item.variant.product.type || "ATLAS LUXE"}</p>
             ...
             <div className="pt-10 border-t border-charcoal/5 space-y-4">
                ...
                {order.discountAmount > 0 && (
                  <div className="flex justify-between text-[10px] uppercase font-bold tracking-widest text-primary underline decoration-primary/20 underline-offset-4 font-serif italic">
                ...
                <div className="flex justify-between pt-6 border-t border-charcoal/5 items-baseline">
                   ...
                   <span className="text-primary text-3xl font-serif italic">₹{Number(order.totalAmount).toLocaleString()}</span>
                </div>
             </div>
          </div>
